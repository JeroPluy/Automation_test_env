...
def _get_trigger_conditional_expression(
    trigger_type: str, entity: Entity, trigger_pos: int, above_position: int = None, source: str = "trigger"
) -> list:
    """
    ...
    if trigger_type == CONF_STATE:
        if entity.expected_value is not None:
            ...
            if isinstance(exp_value, list):
                ...
                if valid_entity_id(str(exp_value)):
                    comparison_str += (
                        f"{array_part}[{trigger_pos}] {operator} {array_part}[{above_position}])"
                    )
                else:
...
def create_combination_trigger_script(
    ...
    Args:
        ...
        filepath (str): the path to the automation script file
        identation_lvl (int, optional): the identation level of the trigger in the automation script. Defaults to 1.
        first_element (bool, optional): the first element of the trigger list. Defaults to True.
        source (str, optional): the source of the entity in the automation script. Defaults to "trigger".
    ...
    if len(entity_list) > 1:
        ...
        for x in range(0, len(entity_list)):
            ...
            if x == 0:
                ...
                script_context += (
                    f"{identation}\t{combinator} (" + trigger_expression[0] + ")\n"
                )
...
def create_trigger_script(
    ...
    if source != "action":
        script_context = identation + IF_TEMPLATE + trigger_expression[0] + END_IF_TEMPLATE
    else:
        script_context = identation + "(" + trigger_expression[0] + ")\n"
...
def create_combination_condition_script(
    ...
    if len(entity_list) > 1:
        # normal condition identation
        if not first_element and identation_lvl < 3:
            script_context += identation + combinator + "(\n"
        elif first_element:
            script_context += identation + "(\n"
        # logical block identation
        else:
            script_context += "(\n"
        ...
        for x in range(0, len(entity_list)):
            ...
            if x == 0:
                ...
                # add the trigger expression to the script with the combinator
                script_context += (
                    f"{identation}{combinator} (" + condition_expression[0] + ")\n"
                )
        ...
        # create the trigger expression for the entities
        condition_expression = _get_condition_expression(
            condition_type=condition_type, entity=entity_list[0], condition_pos=condition_pos, source=source
        )
...
def create_condition_script(
    ...
    # create the trigger expression for the entity
    condition_expression = _get_condition_expression(
        condition_type=condition_type, entity=entity, condition_pos=condition_pos, source=source
    )

    # normal condition identation
    if not first_element and identation_lvl < 3:
        script_context = (
            identation + combinator + " (" + condition_expression[0] + ")\n"
        )
    elif first_element:
        script_context = identation + "(" + condition_expression[0] + ")\n"
    # logical block identation
...
def start_logic_function_block(
    ...
    if condition_type == CONF_NOT:
        script_context += f"{identation}\tnot (\n"

    _append_script_context_to_script(filepath, script_context)
    return identation_lvl + 1


def create_next_logic_condition_part(
    condition_type: str,
    filepath: str,
    identation_lvl: int = 2,
    first_element: bool = True,
) -> None:
    """
...
def close_logic_function_block(
    filepath, condition_type, identation_lvl: int = 2
) -> None:
    """
...
def start_action_condition_block(
    filepath: str, identation_lvl: int = 1, first_element=True, not_condition=False
) -> None:
    """
    ...
    Args:
        filepath (str): The path to the automation script file.
        identation_lvl (int, optional): The identation level of the condition block. Defaults to 1.
        
    Returns:
        int: The new identation level for the conditions in the condition block and the actions afterwards.
    """
    identation = "\t" * identation_lvl

    if not_condition:
        script_context = f"{identation}if not (\n"
    else:
...
def close_action_condition_block(filepath: str, identation_lvl: int = 1) -> None:
    """
    Simple closing of the condition block in the action part of the automation script.
...
def create_stopping_action(filepath: str, identation_lvl: int, timeout: bool) -> None:#
    """
    Closes a condition block and stops the automation script.

    Args:
        filepath (str): The path to the automation script file.
        identation_lvl (int): The identation level of the wait for trigger section in the automation script.
        timeout (bool): Is a timeout for the wait for trigger section in the automation.
    """
    identation = "\t" * identation_lvl

    if timeout:
        script_context = f" or True{END_IF_TEMPLATE}{identation}\tprint(json.dumps(action_results))\n{identation}\treturn\n\n"
    else:
        script_context = f"\n{identation}{END_IF_TEMPLATE}{identation}\tprint(json.dumps(action_results))\n{identation}\treturn\n\n"
    
    _append_script_context_to_script(filepath, script_context)


def create_empty_action_section(filepath: str, identation_lvl: int) -> None:
    """
    Create an empty action section in the automation script.

    Args:
        filepath (str): The path to the automation script file.
        identation_lvl (int): The identation level of the action section in the automation script.
    """
    identation = "\t" * identation_lvl

    script_context = f"{identation}pass\n"
    _append_script_context_to_script(filepath, script_context)


def create_else_action_section(filepath: str, identation_lvl: int) -> None:
    """
    Create the else section in the branching part of the automation script.

    Args:
        filepath (str): The path to the automation script file.
        identation_lvl (int): The identation level of the else section in the automation script.
    """
    identation = "\t" * identation_lvl-1

    script_context = f"{identation}else:\n"
    _append_script_context_to_script(filepath, script_context)
...
def create_action_script(
    ...
    if action_type == CONF_EVENT:
        ...
        script_context = f"{identation}action_results.append({result})\n\n"
    elif action_type == CONF_DEVICE:
        result = "{" + f"'{entity.entity_name}':'{entity.expected_value[CONF_SERVICE]}'" + "}"
        script_context = f"{identation}action_results.append({result})\n\n"
...
def close_action_section(filepath: str) -> None:
    """
    Close the action section in the automation script.
