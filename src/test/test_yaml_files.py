...
def change_param(yaml_dict, param, value="invalid_value", nested = False, nested_param = None) -> dict:
    ...
    validation_result = asyncio.run(ha_automation_config.async_validate_config_item(yaml_dict))
    return [param, validation_result.automation_name, str(validation_result.validation_status), str(validation_result.validation_error)]
    ...
    for param in BASIS_PARAMS:
        ...
        if isinstance(param, list):
            if param[0] == CONF_TRACE:
                ...
                assert change_param(yaml_dict, param[0], -1, True, param[1]) == [param[0], automation_name, "failed_schema", "value must be at least 0 for dictionary value @ data['trace']['stored_traces']. Got -1"]
                # valid value for int
                assert change_param(yaml_dict, param[0], 8, True, param[1]) == [param[0], automation_name, "ok", "None"]
                # test removing stored traces
                assert remove_param(yaml_dict, param[0], True, param[1]) == [param[0], automation_name, "ok", "None"]
                yaml_dict = copy.deepcopy(reset_dict)

                # Test changes to trace parameters only
                assert change_param(yaml_dict, param[0], "invalid_value") == [param[0], automation_name, "failed_schema", "expected a dictionary for dictionary value @ data['trace']. Got 'invalid_value'"]
                # test removing trace
                assert remove_param(yaml_dict, param[0]) == [param[0], automation_name, "ok", "None"]
                yaml_dict = copy.deepcopy(reset_dict)
            ...
            # Test changes to mode parameters only
                ...
                # Test all modes
                for mode in param[1]:
                    assert change_param(yaml_dict, param[0], mode) == [param[0], automation_name, "ok", "None"]

                # Test with invalid mode
                if param[0] == CONF_MODE:
                    assert change_param(yaml_dict, param[0]) == [param[0], automation_name, "failed_schema", "value must be one of ['parallel', 'queued', 'restart', 'single'] for dictionary value @ data['mode']. Got 'invalid_value'"]
                else:
                    assert change_param(yaml_dict, param[0]) == [param[0], automation_name, "failed_schema", "value must be one of ['CRITICAL', 'DEBUG', 'ERROR', 'FATAL', 'INFO', 'NOTSET', 'SILENT', 'WARN', 'WARNING'] for dictionary value @ data['max_exceeded']. Got 'invalid_value'"]
                assert remove_param(yaml_dict, param[0]) == [param[0], automation_name, "ok", "None"]
        else:
            # Test changes to initial state parameter
            if CONF_ID == param:
                # invalid value for str                
                assert change_param(yaml_dict, param, 11054710) == [param, "Invalid automation", "failed_schema", "expected str for dictionary value @ data['id']. Got 11054710"]
            elif CONF_ALIAS == param or CONF_DESCRIPTION == param:
                # invalid value for string
                if CONF_ALIAS == param:
                    assert change_param(yaml_dict, param, None) == [param, "Invalid automation", "failed_schema", "string value is None for dictionary value @ data['alias']. Got None"]
                else:
                    assert change_param(yaml_dict, param, None) == [param, "Invalid automation", "failed_schema", "string value is None for dictionary value @ data['description']. Got None"]
            else:
                # invalid value for int, bool
                result = change_param(yaml_dict, param)
                print(result[0] + " : " + result[1] + " : " + result[2] + " : " + result[3])
            if CONF_ALIAS == param:
                assert remove_param(yaml_dict, param) == [param, "ID: '1600000000000'", "ok", "None"]
            else :
                if CONF_TRIGGER == param or CONF_ACTION == param:
                    result = remove_param(yaml_dict, param)
                    print(result[0] + " : " + result[1] + " : " + result[2] + " : " + result[3])
                else:
                    assert remove_param(yaml_dict, param) == [param, automation_name, "ok", "None"]
